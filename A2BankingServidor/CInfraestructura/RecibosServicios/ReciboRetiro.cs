...
{
    public class ReciboRetiro : IDocument
    {
        private Bank bank { get; }
        private DataRow[] table { get; }
        public ReciboRetiro(Bank banco, DataTable data)
        {
            bank = banco;
            table = data.Select();
        }
        ...
        private void GeneralHeader(IContainer container)
        {
            ...
            container.Row(fila =>
            {
                ...
                fila.RelativeItem().Column(columna =>
                {
                    columna.Item().Scale(letra).Text($"Id Retiro: {table[0]["RetiroId"]}")
        ...
        private void GeneralCuerpo(IContainer container)
        {
            ...
            {
                columna.Item().Text("Retiro a entidad de cuanta Electronica.")
                ...
                columna.Item().Scale(letra).Text("");
                columna.Item().Text("DETALLE DEL RETIRO:").SemiBold().FontSize(sizeLetras)
                ...
                columna.Item().Scale(letra).Text("");
                columna.Item().Text("VALOR RETIRO:").SemiBold().FontSize(sizeLetras)
                .FontFamily("Times New Roman");
